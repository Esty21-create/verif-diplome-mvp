...
import { smtpConfigure } from './envoiEmail.js';
...
// MODE DÉMO : le code est renvoyé au navigateur et affiché à l'écran au lieu
// d'être envoyé par email. Afficher le code à l'écran annule tout l'intérêt de
// l'email (quiconque connaît un matricule pourrait se connecter), donc :
// - MODE_DEMO_CODE_CONNEXION="true"  : démo forcée ;
// - MODE_DEMO_CODE_CONNEXION="false" : envoi réel obligatoire (l'inscription
//   est refusée si le SMTP n'est pas configuré) ;
// - variable absente : envoi réel dès que le SMTP est configuré, démo sinon —
//   configurer le SMTP suffit donc à sortir du mode démo.
export function modeDemoActif() {
  const mode = process.env.MODE_DEMO_CODE_CONNEXION;
